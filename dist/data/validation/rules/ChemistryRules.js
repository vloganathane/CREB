 ...
 */
export class ChemicalFormulaFormatRule extends SyncRule {
    constructor() {
        super('chemical-formula-format', 'Validate basic chemical formula format', { priority: 100, cacheable: true });
    }
    appliesTo(value) {
    ...
    validateSync(value, context) {
        ...
        if (basicPattern.test(formula)) {
            ...
            return this.createFailure('INVALID_FORMULA_FORMAT', `Invalid chemical formula format: ${formula}`, context.path, ValidationSeverity.ERROR, [
 ...
 */
export class ValidElementSymbolsRule extends SyncRule {
    constructor() {
        super('valid-element-symbols', 'Validate that all element symbols in formula are valid', { priority: 90, cacheable: true });
        this.validElements = new Set([
    ...
    validateSync(value, context) {
        ...
        if (invalidElements.length === 0) {
            ...
            return this.createFailure('INVALID_ELEMENT_SYMBOLS', `Invalid element symbols found: ${invalidElements.join(', ')}`, context.path, ValidationSeverity.ERROR, [
                'Check periodic table for correct element symbols',
                'Ensure proper capitalization (e.g., Ca not CA)',
                'Remove any non-element symbols'
            ], { invalidElements, formula }, value);
        }
    }
}
/**
 ...
 */
export class MolecularWeightConsistencyRule extends SyncRule {
    constructor() {
        super('molecular-weight-consistency', 'Validate molecular weight matches formula', { priority: 80, cacheable: true });
        this.atomicWeights = {
    ...
    validateSync(value, context) {
        ...
        try {
            ...
            if (difference <= tolerance) {
                ...
                return this.createFailure('MOLECULAR_WEIGHT_MISMATCH', `Molecular weight mismatch: calculated ${calculatedWeight.toFixed(3)}, provided ${molecularWeight}`, context.path, ValidationSeverity.ERROR, [
            ...
            return this.createFailure('MOLECULAR_WEIGHT_CALCULATION_ERROR', `Cannot calculate molecular weight: ${errorMessage}`, context.path, ValidationSeverity.WARNING, ['Check formula format', 'Verify all elements are recognized'], { error: errorMessage, formula }, value);
        }
    }
 ...
 */
export class GHSClassificationRule extends SyncRule {
    constructor() {
        super('ghs-classification', 'Validate GHS hazard classifications', { priority: 70, cacheable: true });
        this.validGHSCodes = new Set([
    ...
    validateSync(value, context) {
        ...
        if (invalidCodes.length === 0) {
            ...
            return this.createFailure('INVALID_GHS_CODES', `Invalid GHS classification codes: ${invalidCodes.join(', ')}`, context.path, ValidationSeverity.ERROR, [
                'Use valid GHS hazard codes (H200-H420)',
                'Check the latest GHS classification system',
                'Remove any non-standard codes'
            ], { invalidCodes, validCodes: value.filter(code => this.validGHSCodes.has(code)) }, value);
        }
    }
}
/**
 ...
 */
export class FlashPointValidationRule extends SyncRule {
    constructor() {
        super('flash-point-validation', 'Validate flash point is within reasonable range', { priority: 60, cacheable: true });
    }
    appliesTo(value) {
    ...
    validateSync(value, context) {
        ...
        if (value >= minFlashPoint && value <= maxFlashPoint) {
            ...
            return this.createFailure('UNREASONABLE_FLASH_POINT', `Flash point ${value}°C is outside reasonable range [${minFlashPoint}, ${maxFlashPoint}]`, context.path, severity, [
            ...
            ], { value, minFlashPoint, maxFlashPoint }, value);
        }
    }
}
// ============================================================================
 ...
 */
export class PhaseTransitionConsistencyRule extends SyncRule {
    constructor() {
        super('phase-transition-consistency', 'Validate melting point is less than boiling point', { priority: 50, cacheable: true });
    }
    appliesTo(value) {
        return typeof value === 'object' &&
            value !== null &&
    ...
    validateSync(value, context) {
        ...
        if (meltingPoint < boilingPoint) {
            ...
            return this.createFailure('INVALID_PHASE_TRANSITION', `Melting point (${meltingPoint}K) must be less than boiling point (${boilingPoint}K)`, context.path, ValidationSeverity.ERROR, [
                'Verify temperature measurements',
                'Check pressure conditions for measurements',
                'Ensure values are in same units (Kelvin)'
            ], { meltingPoint, boilingPoint }, value);
        }
    }
}
/**
 ...
 */
export class CASNumberValidationRule extends AsyncRule {
    constructor() {
        super('cas-number-validation', 'Validate CAS registry number format and existence', { priority: 40, cacheable: true, timeout: 3000 });
    }
    appliesTo(value) {
    ...
    validateCASFormat(casNumber) {
        const casPattern = /^(\d{2,7})-(\d{2})-(\d)$/;
        const match = casNumber.match(casPattern);
        if (!match) {
            return this.createFailure('INVALID_CAS_FORMAT', `Invalid CAS number format: ${casNumber}`, [], ValidationSeverity.ERROR, ['Use format: NNNNNNN-NN-N where N is a digit'], { casNumber });
        }
        return this.createSuccess({ casNumber, formatValid: true });
    }
    validateCASChecksum(casNumber) {
        ...
        if (calculatedCheck === checkDigit) {
            ...
            return this.createFailure('INVALID_CAS_CHECKSUM', `Invalid CAS number checksum: expected ${calculatedCheck}, got ${checkDigit}`, [], ValidationSeverity.ERROR, ['Verify CAS number is correct', 'Check for transcription errors'], {
                casNumber,
                calculatedCheck,
                providedCheck: checkDigit
            });
...
//# sourceMappingURL=ChemistryRules.js.map